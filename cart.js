...
/* ── LOGO_SRC is defined in each HTML page's ITEM_IMAGES script block ── */
...
/* ── OPEN CHECKOUT MODAL ── */
/* ── SEND ORDER EMAILS (fires when Proceed to Checkout is clicked) ── */
function sendOrderEmails(cart) {
  ...
  // We don't have the customer's details yet at this point —
  // they fill those in on the modal. So we send a preliminary
  // notification to Skylar with just the cart contents,
  // and send the full confirmation to the customer after they
  // fill in their details and click Pay Securely.
  // Store cart snapshot for use after form is filled
  window._pendingOrderLines = orderLines;
  window._pendingTotalStr   = totalStr;
}

function sendConfirmationEmails(name, email, phone, date, street, city, state, zip, other, orderLines, totalStr) {
  orderLines = orderLines || window._pendingOrderLines || '';
  totalStr   = totalStr   || window._pendingTotalStr   || '';

  // ── Email 1: Clean plain-text notification to Skylar ──
  var skylarsPayload = {
  ...
  fetch('https://api.web3forms.com/submit', {
    ...
    body:    JSON.stringify(skylarsPayload)
  }).catch(function(e) { console.warn('Skylar notification failed:', e); });

  // ── Email 2: Send order data to backend — backend builds and sends HTML email ──
  fetch(BACKEND_URL + '/send-confirmation', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify({
      to:         email,
  ...
  }).catch(function(e) { console.warn('Customer confirmation failed:', e); });
}
...
function openCheckout() {
  ...
  closeCartDrawer();
  // Store cart snapshot for emails
  sendOrderEmails(cart);
...
function buildConfirmationEmail(name, email, phone, date, street, city, state, zip, other, orderLines, total, logoSrc) {
  var itemRows = orderLines.split('\n').map(function(line) {
    return (
      '<tr>' +
        '<td style="padding:10px 16px;font-family:Georgia,serif;font-size:15px;font-style:italic;color:#2E3A2E;border-bottom:1px solid #C5C9B0;">' + line.split('=')[0].trim() + '</td>' +
        '<td style="padding:10px 16px;font-family:Georgia,serif;font-size:15px;color:#4A5E3A;text-align:right;border-bottom:1px solid #C5C9B0;">$' + (line.split('= $')[1] || '').trim() + '</td>' +
      '</tr>'
    );
  }).join('');
  ...
  return (
    ...
    '<div style="max-width:600px;margin:0 auto;background:#F5F0E8;">' +
      ...
      /* ── HEADER ── */
      '<div style="background:#4A5E3A;padding:40px 40px 32px;text-align:center;">' +
        '<img src="' + logoSrc + '" alt="Skylar\'s Plant Based Cooking" style="height:80px;object-fit:contain;background:#F5F0E8;padding:8px 16px;border-radius:4px;">' +
        '<p style="font-family:Arial,sans-serif;font-size:10px;letter-spacing:4px;text-transform:uppercase;color:#C5C9B0;margin:20px 0 0;">Order Confirmation</p>' +
      '</div>' +

      /* ── THANK YOU MESSAGE ── */
      '<div style="background:#EAE4D6;padding:40px;text-align:center;border-bottom:3px solid #4A5E3A;">' +
        '<h1 style="font-family:Georgia,serif;font-size:32px;font-weight:400;font-style:italic;color:#2E3A2E;margin:0 0 16px;">Thank You, ' + name.split(' ')[0] + '!</h1>' +
        ...
        '<p style="font-family:Georgia,serif;font-size:16px;font-style:italic;color:#6B7B5A;line-height:1.9;margin:0;">' +
          ...
          'I will personally reach out within 24&ndash;48 hours to confirm your delivery details and answer any questions.' +
          '<br><br>' +
      ...
      '<div style="padding:0 40px 36px;">' +
        ...
        '<table style="width:100%;border-collapse:collapse;background:#EAE4D6;">' +
          '<tr>' +
            '<td style="padding:12px 16px;font-family:Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9AA87A;width:40%;border-bottom:1px solid #C5C9B0;">Name</td>' +
            '<td style="padding:12px 16px;font-family:Georgia,serif;font-size:15px;font-style:italic;color:#2E3A2E;border-bottom:1px solid #C5C9B0;">' + name + '</td>' +
          '</tr>' +
          '<tr>' +
            '<td style="padding:12px 16px;font-family:Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9AA87A;border-bottom:1px solid #C5C9B0;">Email</td>' +
            '<td style="padding:12px 16px;font-family:Georgia,serif;font-size:15px;font-style:italic;color:#2E3A2E;border-bottom:1px solid #C5C9B0;">' + email + '</td>' +
          '</tr>' +
          '<tr>' +
            '<td style="padding:12px 16px;font-family:Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9AA87A;border-bottom:1px solid #C5C9B0;">Phone</td>' +
            '<td style="padding:12px 16px;font-family:Georgia,serif;font-size:15px;font-style:italic;color:#2E3A2E;border-bottom:1px solid #C5C9B0;">' + (phone || 'Not provided') + '</td>' +
          '</tr>' +
          '<tr>' +
            '<td style="padding:12px 16px;font-family:Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9AA87A;border-bottom:1px solid #C5C9B0;">Delivery Date</td>' +
            '<td style="padding:12px 16px;font-family:Georgia,serif;font-size:15px;font-style:italic;color:#2E3A2E;border-bottom:1px solid #C5C9B0;">' + date + '</td>' +
          '</tr>' +
          '<tr>' +
            '<td style="padding:12px 16px;font-family:Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9AA87A;border-bottom:1px solid #C5C9B0;">Delivery Address</td>' +
            '<td style="padding:12px 16px;font-family:Georgia,serif;font-size:15px;font-style:italic;color:#2E3A2E;border-bottom:1px solid #C5C9B0;">' + street + '<br>' + city + ', ' + state + ' ' + zip + '</td>' +
          '</tr>' +
          (other ? (
          '<tr>' +
            '<td style="padding:12px 16px;font-family:Arial,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;color:#9AA87A;">Special Notes</td>' +
            '<td style="padding:12px 16px;font-family:Georgia,serif;font-size:15px;font-style:italic;color:#2E3A2E;">' + other + '</td>' +
          '</tr>'
          ) : '') +
        '</table>' +
      '</div>' +
...
async function handlePayment() {
  ...
  // ── Build order lines directly from cart here (before cart is cleared) ──
  var orderLines = cart.map(function(i) {
    return i.qty + 'x ' + i.name + ' @ $' + i.price + ' = $' + (i.price * i.qty).toFixed(0);
  }).join('\n');
  var totalStr = '$' + cartTotal(cart).toFixed(0);

  // ── Send both emails now that we have all customer details ──
  sendConfirmationEmails(name, email, phone, date, street, city, state, zip, other, orderLines, totalStr);
...
function formatCheckoutPhone(input) {
      ...
      '<div class="stripe-modal">' +
        ...
        '<div id="stripe-modal-content">' +
          ...
          '<div class="stripe-field-row">' +
            ...
            '<div class="stripe-field-group">' +
              '<label>Phone</label>' +
              '<input type="tel" id="checkout-phone" placeholder="(555) 000-0000" maxlength="14" oninput="formatCheckoutPhone(this)">' +
            '</div>' +
          '</div>' +
          '<div class="stripe-field-group">' +
