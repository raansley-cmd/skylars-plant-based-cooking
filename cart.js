...
window.addToCart = function(name, price) {
  ...
  var btns = document.querySelectorAll('.add-to-cart-btn[data-name="' + CSS.escape(name) + '"]');
...
async function handlePayment() {
  ...
  if (result.error) {
    ...
    payBtn.textContent = 'Pay $' + total.toFixed(0) + ' Securely';
    return;
  }

  /* ─────────────────────────────────────────────────────────────
     NOTE TO SKYLAR / DEVELOPER:
     To complete real charges you need a backend server that:
     1. Receives the paymentMethod.id + order details
     2. Creates a PaymentIntent via Stripe API (secret key lives here)
     3. Returns client_secret to confirm the payment

     For now we show the success state and send order via Web3Forms.
     Replace the section below with your backend call when ready.
  ───────────────────────────────────────────────────────────── */

  // Send order summary email via Web3Forms
  ...
  var payload = {
    ...
    'Payment Method ID': result.paymentMethod.id
  };
  ...
  var cartHTML =
    ...
    '<div class="cart-drawer" id="cart-drawer">' +
      ...
      '<div class="cart-drawer-footer" id="cart-drawer-footer" style="display:none;">' +
        ...
        '</div>' +
        '<p class="cart-note">Delivery area: Temecula &amp; surrounding Inland Empire cities &nbsp;&middot;&nbsp; 3 week lead time required</p>' +
    ...
    '<div class="stripe-modal-overlay" id="stripe-modal-overlay">' +
      '<div class="stripe-modal">' +
        ...
        '<div id="stripe-success" style="display:none;" class="stripe-success">' +
          ...
          '<p>Thank you! Skylar will be in touch within 24&ndash;48 hours to confirm your delivery details. Payment will be due upon confirmation.</p>' +
        '</div>' +
        ...
        '<div id="stripe-modal-content">' +
          ...
          '<div class="stripe-card-section">' +
            '<label>Card Details <span style="color:var(--sage)">*</span></label>' +
...
/* ── SET MIN DATE ON CHECKOUT MODAL ── */
function setCheckoutMinDate() {
  var d = new Date();
  d.setDate(d.getDate() + 21);
  var mm = String(d.getMonth()+1).padStart(2,'0');
  var dd = String(d.getDate()).padStart(2,'0');
  var minStr = d.getFullYear() + '-' + mm + '-' + dd;
  var dateInput = document.getElementById('checkout-date');
  if (dateInput) { dateInput.setAttribute('min', minStr); dateInput.value = minStr; }
}
